...
let ShapeDetection = require('./ShapeDetection.js');
let std_msgs = _finder('std_msgs');

//-----------------------------------------------------------

class ShapeDetections {
  constructor(initObj={}) {
    if (initObj === null) {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.detections.forEach((val) => {
      bufferOffset = ShapeDetection.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new ShapeDetections(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.detections[i] = ShapeDetection.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.detections.forEach((val) => {
      length += ShapeDetection.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new ShapeDetections(null);
    ...
    if (msg.detections !== undefined) {
      ...
      for (let i = 0; i < resolved.detections.length; ++i) {
        resolved.detections[i] = ShapeDetection.Resolve(msg.detections[i]);
      }
    }
    else {
      resolved.detections = []
...
module.exports = ShapeDetections;
